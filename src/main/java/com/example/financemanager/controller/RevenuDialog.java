...
public class RevenuDialog extends Dialog<Revenu> {
    @FXML
    ...
    @FXML
    private TextField selfEmployedField;

    @FXML
    private TextField passifField;
    ...
    public RevenuDialog() {
        try {
            FXMLLoader loader = new FXMLLoader(FinanceTrackerApplication.class.getResource("revenu-dialog.fxml"));
    ...
    private void forceDateFormat() {
        ...
        periodField.setTextFormatter(new TextFormatter<>(dateValidationFormatter));
    }
    ...
    private void forceDoubleFormat() {
        ...
        selfEmployedField.setTextFormatter(new TextFormatter<>(numberValidationFormatter));
        passifField.setTextFormatter(new TextFormatter<>(numberValidationFormatter));
    ...
    private void computeIfButtonIsDisabled() {
        getDialogPane().lookupButton(createButton).disableProperty().bind(
                Bindings.createBooleanBinding(() -> periodField.getText().trim().isEmpty(), periodField.textProperty())
                        .or(Bindings.createBooleanBinding(() -> salaryField.getText().trim().isEmpty(), salaryField.textProperty())
                                .or(Bindings.createBooleanBinding(() -> helpField.getText().trim().isEmpty(), helpField.textProperty())
                                        .or(Bindings.createBooleanBinding(() -> selfEmployedField.getText().trim().isEmpty(), selfEmployedField.textProperty())
                                                .or(Bindings.createBooleanBinding(() -> passifField.getText().trim().isEmpty(), passifField.textProperty())
    ...
    private void initResultConverter() {
        setResultConverter(buttonType -> {
            ...
            return new Revenu(
                    LocalDate.parse(periodField.getText(), DateTimeFormatter.ofPattern("dd/MM/yy")),
                    Float.parseFloat(salaryField.getText()),
                    Float.parseFloat(helpField.getText()),
                    Float.parseFloat(selfEmployedField.getText()),
                    Float.parseFloat(passifField.getText()),
